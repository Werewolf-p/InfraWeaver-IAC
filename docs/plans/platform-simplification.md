...
| C3 | ✅ **DONE 2026-08-19 — deleted from infra.** Two earlier passes disagreed on ownership; both premises were wrong. Measured: `InfraWeaver-platform` is **PRIVATE**, so the README:88 one-liner `raw.githubusercontent.com/…/InfraWeaver-platform/main/scripts/init/setup.sh` **404s for everyone**. `sync-to-public.sh` does publish `scripts/`, so infra's nested tree WAS live on the PUBLIC mirror — but the mirror's own README quick-start names `scripts/setup.sh` (the .env wizard), never `scripts/init/**`, and the mirror has no `apps/` tree, so `build-ui.sh` could never work there at any nesting depth. Infra's copy also defaulted `IW_REPO_URL`/`RAW_BASE` to `github.com/your-org/your-repo` — **a real, unrelated third party's repo** — and lacked platform's `_validate_outbound_host` SSRF guard (platform `bf2c9033`, 2026-07-05) while shipping `Authorization: PVEAPIToken=` to a caller-supplied host. A `git mv` would have fixed 2 of those 4 defects and closed 0 duplicate pairs (the gate matches basenames). Deleted instead | `curl` raw URLs: platform 404 / mirror `scripts/init/init/*` 200; `gh repo view` visibility; `grep -c _validate_outbound_host` = 0 infra / 5 platform | Done. **Open for a human:** ~10 infra-only wizard commits (dynamic node count, `/api/latest-versions`, LAN-allowlist autodetect, Talos factory schematic, derived VIPs, node roles) are NOT in platform's copy — port them deliberately or accept the loss; recover from `ce5bcea:scripts/init/init/` | 6,898 |
