...
# ArgoCD-native Helm does not run envsubst, so a ${BASE_DOMAIN} in argo-cd's
# values.yaml reaches argocd-cm.url literally and crashes argocd-server. Inject
# global.domain as a Helm param scoped (inline) to the argocd app — this AppSet is
# rendered through the envsubst CMP plugin, so ${BASE_DOMAIN} resolves at deploy.
# Non-argocd core apps get global.domain="" (inert; cert-manager etc. tolerate it).
ARGOCD_DOMAIN_PARAM = (
    '              - name: global.domain\n'
    '                value: \'{{ if eq .path.path "kubernetes/core/argocd" }}argocd.int.${BASE_DOMAIN}{{ end }}\'\n'
)
...
core_content = appset_yaml(
    ...
    extra_params=ARGOCD_DOMAIN_PARAM
)
