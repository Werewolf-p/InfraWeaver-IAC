...
if (action === 'suggest') { const resp = await cfReq.call(this, 'GET', `${baseUrl}?per_page=100&type=A`); const existing = new Set((resp.result||[]).map(r => r.name)); const services = ['grafana','prometheus','longhorn','argocd','openbao','wiki','gitea','vaultwarden','forgejo','it-tools','stirling-pdf','excalidraw','actual']; const suggestions = services.filter(s => !existing.has(`${s}.int.example.com`)).map(s => ({ name:`${s}.int.example.com`, type:'A', content:'10.0.0.200', proxied:false, note:'Covered by wildcard but explicit record improves monitoring visibility' })); return [{json: {status:'ok', suggestions, total:suggestions.length}}]; }
